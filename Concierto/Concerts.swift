...
class Concerts {
    ...
    func getData(state: String,completed: @escaping () -> ()) {
        let url = "https://api.seatgeek.com/2/events?venue.state=\(state)&client_id=\(APIkeys.seatGeekKey)"
        ...
        let task = session.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                for data in returned.events {
                    if data.performers.first?.image == "<null>" {
                        self.concertArray.append(ConcertData(venue: data.venue.name, name: data.performers.first?.name ?? "", location: data.venue.display_location, type: data.type, url: data.url, image: "https://imgur.com/a/nfZBHFa", date: data.datetime_utc, state: data.venue.state))
                    } else {
